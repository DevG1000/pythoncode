...
class SecurityMonitor:
    ...
    def _record_login_failure(self, log_line: str):
        ...
        event = SecurityEvent(
            ...
            details={
                "log_line": log_line,
                "ip_address": ip,
                "count": self.metrics[f"login_failures_{ip}"] + 1
            }
        )
    ...
    def _check_login_failure_alerts(self):
        ...
        recent_failures = [
            ...
            and datetime.fromisoformat(e.timestamp) > five_minutes_ago
        ]
...
def main():
    ...
    parser.add_argument('--config', default='config/security-monitor.yaml',
                       help='监控配置文件路径')
    parser.add_argument('--daemon', action='store_true',
                       help='以守护进程模式运行')
    parser.add_argument('--status', action='store_true',
                       help='显示监控状态')
    parser.add_argument('--report', choices=['hourly', 'daily'],
                       help='生成报告')
