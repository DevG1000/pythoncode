...
class ProductionDeployment:
    ...
    def create_backup(self):
        ...
        if os.path.exists(deploy_dir):
            ...
            success, _, _ = self.run_command(f"cp -r {deploy_dir} {backup_path}", "Copying deployment directory")
    ...
    def build_docker_images(self):
        ...
        if os.path.exists(docker_compose_file):
            success, output, error = self.run_command(
                f"docker-compose -f {docker_compose_file} build", "Building Docker images"
            )
            ...
            if os.path.exists("docker-compose.yml"):
                success, output, error = self.run_command(
                    "docker-compose build", "Building Docker images with default compose file"
                )
                return success
            else:
    ...
    def deploy_application(self):
        ...
        for item in items_to_copy:
            ...
            if os.path.exists(source):
                ...
                if os.path.isdir(source):
                    # Copy directory
                    self.run_command(f"cp -r {source} {dest}", f"Copying directory: {item}")
                else:
                    # Copy file
                    self.run_command(f"cp {source} {dest}", f"Copying file: {item}")
            else:
        ...
        if os.path.exists(config_file):
            ...
            self.run_command(f"cp {config_file} {dest_config}", f"Setting {self.environment} configuration")
    ...
    def start_services(self):
        ...
        try:
            ...
            if os.path.exists(docker_compose_file):
                success, output, error = self.run_command(
                    f"docker-compose -f {docker_compose_file} up -d", "Starting services with Docker Compose"
                )
            elif os.path.exists("docker-compose.yml"):
                success, output, error = self.run_command(
                    "docker-compose up -d", "Starting services with default Docker Compose"
                )
            else:
                ...
                success, output, error = self.run_command(
                    "cd api && python app.py &", "Starting API service manually", check=False
                )
            ...
            if success:
                ...
                success, output, error = self.run_command(
                    f"curl -f http://localhost:{port}/health", "Checking service health", check=False
                )
    ...
    def run_post_deployment_checks(self):
        ...
        success, output, error = self.run_command(
            f"curl -s http://localhost:{port}/health", "Checking service health endpoint", check=False
        )

        if success and "healthy" in output.lower():
            checks.append(("Service Health", "[OK]"))
        else:
        ...
        if success and "pythoncode" in output:
            checks.append(("Docker Containers", "[OK]"))
        else:
    ...
    def deploy(self):
        ...
        try:
            ...
            if all([self.prerequisites_passed, self.tests_passed, self.deployment_successful, self.services_started]):
                self.log("Deployment successful! [OK]", level="SUCCESS")
                self.log(f"Report: {report_file}")
                return True
            else:
                self.log("Deployment completed with issues [WARN]️", level="WARNING")
                self.log(f"Report: {report_file}")
                return False
...
def main():
    ...
    parser.add_argument("--dry-run", action="store_true", help="Dry run - show what would be done without actually deploying")
