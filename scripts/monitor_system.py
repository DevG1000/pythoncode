...
class SystemMonitor:
    ...
    def load_config(self) -> Dict[str, Any]:
        ...
        default_config = {
            "monitoring": {
                ...
                "metrics": {
                    "system": {"cpu_threshold": 80, "memory_threshold": 85, "disk_threshold": 90},
                    "application": {"response_time_threshold": 5000, "error_rate_threshold": 5},
                },
            }
        }
    ...
    def collect_system_metrics(self) -> Dict[str, Any]:
        """收集系统指标"""
        metrics = {"timestamp": datetime.now().isoformat(), "system": {}, "application": {}, "security": {}}
        ...
        try:
            ...
            metrics["system"]["top_processes"] = sorted(processes, key=lambda x: x.get("cpu_percent", 0), reverse=True)[:10]
    ...
    def check_application_health(self) -> Dict[str, Any]:
        """检查应用健康状态"""
        health = {"timestamp": datetime.now().isoformat(), "status": "unknown", "checks": []}
        ...
        try:
            ...
            health["checks"].append({"name": "api_health", "status": "unreachable", "error": str(e)})
        ...
        try:
            ...
            if db_url:
                ...
                health["checks"].append({"name": "database", "status": "healthy"})
            else:
                health["checks"].append({"name": "database", "status": "not_configured"})

        except Exception as e:
            health["checks"].append({"name": "database", "status": "unhealthy", "error": str(e)})
        ...
        try:
            ...
            health["checks"].append({"name": "redis", "status": "skipped", "message": "Redis检查已跳过"})

            health["checks"].append({"name": "redis", "status": "healthy"})

        except Exception as e:
            health["checks"].append({"name": "redis", "status": "unhealthy", "error": str(e)})
    ...
    def check_security_metrics(self) -> Dict[str, Any]:
        """检查安全指标"""
        security = {"timestamp": datetime.now().isoformat(), "checks": [], "alerts": []}
        ...
        try:
            ...
            if os.path.exists(log_file):
                ...
                security["checks"].append({"name": "failed_logins", "count": failed_logins, "period": "1h"})
            ...
            if os.path.exists(file_path):
                try:
                    ...
                    if mode & 0o777 == 0o777:  # rwxrwxrwx
                        insecure_files.append({"file": file_path, "permissions": oct(mode)})
                except Exception:
                    pass

        security["checks"].append({"name": "file_permissions", "insecure_files": insecure_files})
...
"""
    ...
    def run_monitoring_cycle(self):
        ...
        try:
            ...
            print(
                f"系统状态: CPU={metrics['system'].get('cpu_percent', 0)}%, "
...
def main():
    ...
    parser.add_argument("--once", action="store_true", help="运行一次监控")
    parser.add_argument("--interval", type=int, default=300, help="监控间隔（秒）")
    parser.add_argument("--config", default="config/monitoring-config.yaml", help="监控配置文件路径")
