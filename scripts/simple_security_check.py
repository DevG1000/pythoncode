...
class SecurityChecker:
    ...
    def _load_checks(self):
        ...
        self.checks = [
            ...
            {"id": "check-003", "description": "检查数据库SSL配置", "severity": "high", "function": self._check_database_ssl},
            {
                "id": "check-004",
            ...
            {"id": "check-005", "description": "检查开放端口", "severity": "medium", "function": self._check_open_ports},
            {"id": "check-006", "description": "检查依赖包安全", "severity": "high", "function": self._check_dependencies},
            {
                "id": "check-007",
                        ...
                        if pattern in line:
                            ...
                            if value and value not in [
                                ...
                                if not (value.startswith("${") and value.endswith("}")):
                                    secrets_found.append({"pattern": pattern, "line": line.strip()})

        if secrets_found:
            return {"status": "failed", "message": f"发现 {len(secrets_found)} 个可能的硬编码密钥", "details": secrets_found}
        else:
            return {"status": "passed", "message": "未发现硬编码密钥", "details": []}
    ...
    def _check_flask_environment(self) -> Dict[str, Any]:
        ...
        if issues:
            return {"status": "failed", "message": "Flask环境配置不安全", "details": issues}
        else:
            return {"status": "passed", "message": "Flask环境配置正确", "details": []}
    ...
    def _check_database_ssl(self) -> Dict[str, Any]:
        ...
        if not db_url:
            return {"status": "warning", "message": "未设置DATABASE_URL环境变量", "details": []}
        ...
        if ssl_enabled:
            return {"status": "passed", "message": "数据库连接已启用SSL", "details": [db_url]}
        else:
            return {"status": "failed", "message": "数据库连接未启用SSL", "details": [db_url]}
    ...
    def _check_log_permissions(self) -> Dict[str, Any]:
        ...
        if issues:
            ...
            return {"status": "passed", "message": "日志文件权限配置正确", "details": []}

    def _check_open_ports(self) -> Dict[str, Any]:
        """检查开放端口"""
        try:
            # 使用netstat检查开放端口
            result = subprocess.run("netstat -an | findstr LISTENING", shell=True, capture_output=True, text=True, timeout=10)
            ...
            if suspicious_ports:
                ...
                return {"status": "passed", "message": "端口配置正确", "details": open_ports[:10]}  # 只显示前10个

        except Exception as e:
            return {"status": "warning", "message": f"端口检查失败: {str(e)}", "details": []}
    ...
    def _check_dependencies(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if not os.path.exists(req_file):
                return {"status": "warning", "message": "未找到requirements.txt文件", "details": []}
            ...
            if found_insecure:
                ...
                return {"status": "passed", "message": "依赖包检查通过", "details": [f"共检查 {len(dependencies)} 个依赖"]}

        except Exception as e:
            return {"status": "warning", "message": f"依赖检查失败: {str(e)}", "details": []}
    ...
    def _check_sensitive_info_in_code(self) -> Dict[str, Any]:
                                        ...
                                        if "=" in line and not line.strip().startswith("#"):
                                            ...
                                            if len(parts) == 2:
                                                ...
                                                if (value.startswith('"') and value.endswith('"')) or (
                                                    ...
                                                    # 检查是否是占位符
                                                    if inner_value and inner_value not in [
                                                        "",
                                                        "changeme",
                                                        "your_password",
                                                        "your-secret-key",
        ...
        if issues:
            ...
            return {"status": "passed", "message": f"代码检查通过，检查了 {checked_files} 个文件", "details": []}
    ...
    def _print_text_report(self, report: Dict[str, Any]):
        ...
        for result in report["results"]:
            status_symbol = {"passed": "[PASS]", "failed": "[FAIL]", "warning": "[WARN]", "error": "[ERR]"}.get(
                result["status"], "[UNK]"
            )
