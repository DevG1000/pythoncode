    ...
    for env_var, config_path in env_mappings.items():
        if env_var in os.environ:
            ...
            if (
                ...
                or env_var.endswith("_SIZE")
                or env_var.endswith("_COUNT")
                or env_var.endswith("_TIMEOUT")
                or env_var.endswith("_EXPIRY")
                or env_var.endswith("_INTERVAL")
                or env_var.endswith("_MB")
            ):
                try:
                    value = int(value)
                except ValueError:
                ...
                env_var.endswith("_DEBUG")
                or env_var.endswith("_TLS")
                or env_var.endswith("_VALIDATION")
                or env_var.endswith("_MONITORING")
                or env_var.endswith("_ECHO")
                or env_var.endswith("_SWAGGER")
            ):
...
def validate_config(config: Dict) -> Dict[str, Any]:
    ...
    """
    results = {"valid": True, "errors": [], "warnings": [], "environment": config.get("app", {}).get("environment", "unknown")}
...
def print_config_summary(config: Dict, validation: Dict = None):
    ...
    print(f"\nEnvironment: {env}")
    print(f"Valid: {'[OK]' if validation['valid'] else '[FAIL]'}")
...
def main():
    ...
    parser.add_argument("--environment", "-e", default=None, help="Environment name (development, staging, production)")
    parser.add_argument("--validate", "-v", action="store_true", help="Validate configuration")
    parser.add_argument("--save", "-s", action="store_true", help="Save configuration to file")
    parser.add_argument("--format", "-f", default="json", choices=["json", "yaml"], help="Output format for save")
