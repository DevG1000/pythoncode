...
class DesignPrincipleChecker:
    def __init__(self):
        ...
        self.stats = {"files_analyzed": 0, "classes_found": 0, "methods_found": 0, "functions_found": 0}
    ...
    def _check_class(self, node: ast.ClassDef, filepath: Path):
        ...
        # SRP检查：类方法过多
        if len(methods) > 12:
            self.issues["SRP"].append(f"{filepath}: 类 '{class_name}' 有 {len(methods)} 个方法，" "可能违反单一职责原则")
    ...
    def _check_method(self, node: ast.FunctionDef, class_name: str, filepath: Path):
        ...
        if args_count > 6:
            self.issues["ISP"].append(
                f"{filepath}: 方法 '{class_name}.{method_name}' 有 {args_count} 个参数，" "可能违反接口隔离原则"
            )
        ...
        if method_text:
            ...
            if lines > 80:
                self.issues["SRP"].append(
                    f"{filepath}: 方法 '{class_name}.{method_name}' 过长 ({lines} 行)，" "可能违反单一职责原则"
                )
    ...
    def _check_function(self, node: ast.FunctionDef, filepath: Path):
        ...
        args_count = len(node.args.args)
        if args_count > 8:
            self.issues["ISP"].append(f"{filepath}: 函数 '{func_name}' 有 {args_count} 个参数，" "可能违反接口隔离原则")
        ...
        if func_text:
            ...
            if lines > 100:
                self.issues["SRP"].append(f"{filepath}: 函数 '{func_name}' 过长 ({lines} 行)，" "可能违反单一职责原则")
                            ...
                            for low_module in low_level_modules:
                                if f"import {low_module}" in content or f"from {low_module}" in content:
                                    self.issues["DIP"].append(
                                        f"{filepath}: 直接导入低层模块 '{low_module}'，" "可能违反依赖倒置原则"
                                    )
    ...
    def generate_report(self, output_file=None, format="text"):
        ...
        for principle in principles:
            ...
            # 根据问题数量计算分数（问题越少分数越高）
            if issue_count == 0:
                score = max_score_per_principle
            elif issue_count <= 3:
                score = max_score_per_principle - 5
            elif issue_count <= 10:
                score = max_score_per_principle - 10
            elif issue_count <= 20:
                score = max_score_per_principle - 15
            else:
                score = max_score_per_principle - 20

            scores[principle.lower() + "_score"] = max(0, score)
        ...
        if format == "text":
            ...
            for principle in principles:
                ...
                if issues:
                    print(f"\n{principle}原则问题 ({issue_count}个):")
                    for issue in issues[:5]:  # 只显示前5个
                        print(f"  [ISSUE] {issue}")
                    if issue_count > 5:
                        print(f"  ... 还有{issue_count-5}个问题未显示")
                else:
                    print(f"\n{principle}原则: [OK] 未发现问题")

            print("\n" + "=" * 70)
            print(f"总计发现 {total_issues} 个设计原则问题")
            print(f"SOLID原则评分: {total_score}/100")
...
def main():
    """主函数"""
    parser = argparse.ArgumentParser(description="检查PythonCode项目的SOLID原则遵循情况")
    parser.add_argument("--output", "-o", help="输出JSON报告文件路径")
    parser.add_argument("--format", "-f", choices=["text", "json"], default="text", help="输出格式 (text 或 json)")
    parser.add_argument("--path", "-p", default=".", help="要检查的项目路径")
    ...
    if args.format == "json" and not args.output:
        args.output = "quality-report.json"

    scores = checker.generate_report(args.output, args.format)

    # 如果只输出JSON，显示简要信息
    if args.format == "json" and args.output:
        print(f"JSON报告已生成: {args.output}")
        print(f"SOLID原则评分: {scores.get('total_score', 0)}/100")
