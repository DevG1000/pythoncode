...
"""
    ...
    def run_security_audit(self):
        ...
        try:
            result = subprocess.run(
                ["python", "scripts/security_audit_simple.py", "--format", "text"], capture_output=True, text=True
            )
    ...
    def display_summary(self):
        ...
        print("\n3. 创建的目录:")
        directories = ["logs/", "logs/nginx/", "uploads/", "config/nginx/ssl/", "config/grafana/provisioning/"]
        for directory in directories:
...
def main():
    ...
    parser.add_argument("--env-file", default=".env.production", help="环境变量文件路径")
