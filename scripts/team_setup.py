    ...
    def setup_development_environment(self):
        ...
        for req_file in requirements_files:
            if (self.project_root / req_file).exists():
                pip_cmd = (
                    [str(self.project_root / "venv" / "Scripts" / "python"), "-m", "pip", "install", "-r", req_file]
                    if os.name == "nt"
                    else [str(self.project_root / "venv" / "bin" / "python"), "-m", "pip", "install", "-r", req_file]
                )
...
"""
    ...
    def setup_vscode_settings(self):
        ...
        settings = {
            "python.defaultInterpreterPath": (
                "${workspaceFolder}/venv/Scripts/python.exe" if os.name == "nt" else "${workspaceFolder}/venv/bin/python"
            ),
    ...
    def generate_team_report(self):
        ...
        print(f"  [OK] 团队报告已生成: {report_path}")
...
"""
        ...
        print("  [OK] GitHub Actions工作流设置完成")
